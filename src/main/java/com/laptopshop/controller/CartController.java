...
public class CartController {
	...
	public String cartPage(HttpServletRequest res,Model model) {
		...
		if(auth == null || auth.getPrincipal() == "anonymousUser")     //Lay tu cookie
		{
			...
			listsp = getSanPhams(quanity, cl, idList, sanPhamService);
		}else     //Lay tu database
		{
			getGioHang(currentUser, quanity, listsp, gioHangService, chiMucGioHangService);
		}
	...
	static void getGioHang (NguoiDung currentUser, Map<Long, String> quanity, List<SanPham> listsp, GioHangService gioHangService, ChiMucGioHangService chiMucGioHangService) {
	...
	static List<SanPham> getSanPhams (Map<Long, String> quanity, Cookie[] cl, Set<Long> idList, SanPhamService sanPhamService) {
		List<SanPham> listsp;
